...
def get_endpoint_config(path, method, is_response):
    with app.app_context():
        ...
        return {
            field.name: {
                'method': field.anonymization.anonymization_method.name,
                'data_category': field.data_category,
                'field_id': field.id  # Dodajemy ID pola!
            }
            for field in endpoint.fields
            if (field.is_response_field == is_response and
                field.anonymization and
...
#             #print(field, data_category)
#             print(key, field, value, data_category)
...
def anonymize_item(item, field_config, service_uuid):
    ...
    for key, value in item.items():
        if key in field_config:
            ...
            # Pobierz pole po ID (najszybsza i najbezpieczniejsza metoda)
            field = Field.query.get(config['field_id'])
            
            data_category = field.data_category if field else None
            anonymized[key] = apply_anonymization(
                value, 
                field_config[key], 
                encryption_key, 
                data_category
